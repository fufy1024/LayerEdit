...
class sdxl(StableDiffusionXLPipeline): 
        ...
        **kwargs
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t).to(self.unet.dtype)
        ...
        img_all=[]
        for i_ in range(latents.shape[0]):
            imgs_ = self.decode_latents(latents[i_].unsqueeze(0).type(torch.cuda.HalfTensor)) 
            img_all.append ( T.ToPILImage()(imgs_[0].cpu()) )
        return img_all
    ...
    def decode_latents(self, latents):
        ...
        imgs = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
